...
def experiment(args):
    ...
    for model_name in ["GPT4", "Gemini", "Claude"]:
        for setting_length in [300]:
            data_file = f"/data/zhuxiaowei/project/AIDetection/Related_dataset/DSB/DSB_machine_test_v4.json"
            with open(data_file, 'r+', encoding='utf-8') as file:
                data = json.load(file)
                # human_text_list = data.get("human_text", [])
                human_text_list = data.get("machine_text", [])
                text_list = human_text_list[4500:5000]
                ori_text = []
                predictions = []
                all_length = []
                ...
                for each_text in tqdm(text_list):
                    if type(each_text) != float:
                        ...
                        tokenized = scoring_tokenizer(each_text, return_tensors="pt", padding=True,
                                                      ...
                                                      max_length=setting_length).to(device)
            ...
            with open(
                    f'/data/zhuxiaowei/project/AIDetection/DNA-DetectLLM/scores/length_truncation/{model_name}_machine_test_lastde_{setting_length}.json',
                    'w') as out1:
